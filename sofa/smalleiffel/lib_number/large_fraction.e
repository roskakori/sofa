...
feature {NUMBER} -- To convert a LARGE_FRACTION  
...
feature {NUMBER} -- Opposite    
   ...
   prefix "-": NUMBER is  
	 -- Opposite of 'Current'.  
      do
	 !LARGE_FRACTION!Result.make_simply(numerator,denominator,not is_negative);
      end;  
...
feature {NUMBER} -- Inverse   
   ...
   inverse: NUMBER is  
	 ...
	 elseif (-numerator).is_one then
	    ...
	    !LARGE_FRACTION!Result.make_simply(denominator, numerator, is_negative);
	 end;
      end;  
...
invariant  
   ...
   not (numerator.is_integer and denominator.is_integer);
