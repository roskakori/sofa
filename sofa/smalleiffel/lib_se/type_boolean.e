...
feature
   ...
   actual_reference(destination: TYPE): TYPE is
      local
         cn: CLASS_NAME;
      do
         if destination.run_time_mark = as_boolean_ref then
            Result := destination;
         else
            !!cn.make(as_boolean_ref,destination.start_position);
   ...
   smallest_ancestor(other: TYPE): TYPE is
      ...
      do
         ...
         if rto.is_boolean then
            ...
            Result := type_boolean_ref.smallest_ancestor(rto);
         end;
      end;
...
feature {NONE}
   ...
   type_boolean_ref: TYPE_CLASS is
      ...
      once
         !!boolean_ref.unknown_position(as_boolean_ref);
         !!Result.make(boolean_ref);
      end;

   check_type is
         -- Do some checking for type BOOLEAN.
      local
         bc: BASE_CLASS;
         rc: RUN_CLASS;
      once
         ...
         if nb_errors = 0 then
            rc := run_class;
            load_ref(as_boolean_ref);
         end;
         if nb_errors = 0 then
            if not bc.is_expanded then
               error(start_position,"BOOLEAN must be expanded.");
            end;
         end;
      end;
