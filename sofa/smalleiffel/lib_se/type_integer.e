...
feature
   ...
   make(sp: like start_position) is
      do
         !!base_class_name.make(as_integer,sp);
      end;
   ...
   actual_reference(destination: TYPE): TYPE is
      local
         cn: CLASS_NAME;
      do
         if destination.run_time_mark = as_integer_ref then
            Result := destination;
         else
            !!cn.make(as_integer_ref,destination.start_position);
...
feature {NONE}
   ...
   type_integer_ref: TYPE_CLASS is
      ...
      once
         !!integer_ref.unknown_position(as_integer_ref);
